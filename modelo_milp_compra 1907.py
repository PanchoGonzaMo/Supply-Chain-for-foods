...
class ModelConfig:
    ...
    # ---- Stock de seguridad: cotas de escala -------------------------------
    # Tope general, en meses de venta. Protege contra un σ mal escalado
    # (p. ej. RMSE agregado aplicado a un SKU de baja rotación). 0 = sin tope.
    ss_max_meses_venta: float = 3.0
    # Piso mínimo, en meses de venta. Permite fijar un colchón por decisión de
    # negocio, independiente de la calidad del pronóstico. 0 = sin piso.
    ss_min_meses_venta: float = 0.0
    # TOPE POR VIDA ÚTIL — el más importante.
    # El stock de seguridad es inventario que permanece en bodega. Si excede lo
    # que alcanza a rotar dentro de la vida útil residual, el propio SS GARANTIZA
    # vencimientos: obliga a comprar producto que caducará antes de venderse.
    # Se acota a una fracción de la demanda consumible dentro de la vida útil.
    ss_fraccion_vida_util: float = 0.5
    vu_slack_penalty: float = 5.0e3       # penaliza exceder la ventana de vida útil
...
def solve_milp(
    ...
    # ---- variables ----
    n, Q, I, S, delta, k_s, k_f, F, ss_short, Pst, vu_slack = ({} for _ in range(11))
    ...
    for sk in skus:
        ...
        for t in periods:
            ...
            # holgura de la restricción de vida útil (Ec. 11): absorbe el
            # sobre-stock HEREDADO, que el modelo no decidió
            vu_slack[p, t]  = pulp.LpVariable(f"vus_{p}_{t}", lowBound=0, cat="Continuous")
    ...
    # --- Salvaguarda de escala del stock de seguridad -------------------------
    # SS = Z·σ·√L es válido solo si σ está en la ESCALA DEL SKU. Cuando σ proviene
    # de un fallback agregado (p. ej. el RMSE del forecast nacional de la sección
    # 10), aplicarlo a un SKU de baja rotación produce un SS de decenas de meses
    # de venta: el modelo intenta sostener un piso imposible y el problema se
    # vuelve infactible o compra de más.
    # El tope acota el SS a un número razonable de meses de venta y deja registro
    # de los SKU afectados, que son los que tienen un σ mal escalado.
    ss_ajustes = {}
    for sk in skus:
        p, VU = sk.sku_id, int(sk.shelf_life_months)
        d = Dhat[p]
        d_mes = float(np.mean(d)) if len(d) else 0.0
        ss_teorico = SS[p]
        motivo = None

        # (a) TOPE POR VIDA ÚTIL: el SS no puede exceder lo que rota dentro de VU.
        #     Sin esta cota, el propio piso de seguridad obliga a comprar producto
        #     que vencerá antes de venderse.
        if cfg.ss_fraccion_vida_util and VU > 0 and len(d):
            consumible_vu = float(np.sum(d[:VU]))
            tope_vu = cfg.ss_fraccion_vida_util * consumible_vu
            if SS[p] > tope_vu:
                SS[p] = tope_vu; motivo = "vida útil"

        # (b) tope general en meses de venta (protege de un σ mal escalado)
        if cfg.ss_max_meses_venta and cfg.ss_max_meses_venta > 0 and d_mes > 0:
            tope = cfg.ss_max_meses_venta * d_mes
            if SS[p] > tope:
                SS[p] = tope; motivo = "σ mal escalado" if motivo is None else motivo + " + σ"

        # (c) piso mínimo por decisión de negocio (se aplica al final, pero nunca
        #     por sobre el tope de vida útil: no tiene sentido un piso que vence)
        if cfg.ss_min_meses_venta and cfg.ss_min_meses_venta > 0 and d_mes > 0:
            piso = cfg.ss_min_meses_venta * d_mes
            if VU > 0 and len(d):
                piso = min(piso, cfg.ss_fraccion_vida_util * float(np.sum(d[:VU])))
            if SS[p] < piso:
                SS[p] = piso; motivo = "piso mínimo"

        if motivo:
            ss_ajustes[p] = {"ss_teorico": ss_teorico, "ss_aplicado": SS[p],
                             "motivo": motivo,
                             "meses_venta": (SS[p]/d_mes if d_mes > 0 else float("inf")),
                             "vida_util": VU}
    ss_capped = ss_ajustes   # compatibilidad
    if ss_ajustes and cfg.solver_msg:
        print(f"  [SS] {len(ss_ajustes)} SKU con stock de seguridad ajustado")
    ...
    for sk in skus:
        ...
        for t in periods:
            ...
            # exceso sobre la ventana de vida útil: fuertemente penalizado, para
            # que el modelo nunca AGREGUE stock no consumible, pero sin volver
            # infactible el problema cuando el exceso ya viene heredado.
            obj.append(cfg.vu_slack_penalty * vu_slack[p, t])
    prob += pulp.lpSum(obj)
    ...
    for sk in skus:
        ...
        for t in periods:
            ...
            #
            # La restricción es BLANDA. Motivo: `inv_prev` incluye, en t=1, el
            # inventario HEREDADO, que no es una decisión del modelo —ya está en
            # bodega—. Si ese stock inicial excede la ventana de vida útil (una
            # situación real de sobre-stock), una restricción dura haría el
            # problema INFACTIBLE y el modelo no entregaría plan alguno.
            # Con holgura penalizada, el exceso heredado se absorbe y se REPORTA,
            # pero el modelo sigue impedido de AGREGAR inventario no consumible:
            # cualquier unidad extra paga la penalización.
            window = [Dhat[p][k - 1] for k in range(t, min(t + VU, T + 1))]
            prob += (inv_prev + arr_conf + arr_order
                     <= pulp.lpSum(window) + vu_slack[p, t]), f"shelf_{p}_{t}"
    ...
    for sk in skus:
        ...
        for t in periods:
            ...
            rows.append({
                ...
                "exceso_vida_util": round(vu_slack[p, t].value() or 0.0),
    ...
    return {
        ...
        "resultados": df,
        "safety_stock": SS,
        "ss_ajustes": ss_ajustes,
        "ss_acotados": ss_capped,
